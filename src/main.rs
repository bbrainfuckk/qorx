...
impl DoctorReport {
    async fn collect() -> Result<Self> {
        ...
        Ok(Self {
            ...
            note: "This is the public Qorx CLI: language, local indexing, pack/squeeze, strict evidence, and source-built proof commands. Qorx Void adds the signed app experience, tray, auto-start, provider routing, one-click integrations, updater, and managed local-vault UX. Qorx Cloud adds hosted account features.".to_string(),
        })
    }
    ...
    fn print_human(&self) {
        ...
        println!("ayie_services: use Qorx Void for tray, daemon, provider routing, integrations, updater, and managed local-vault UX");
...
fn pro_only(feature: &str) -> Result<()> {
    anyhow::bail!(
        "{feature} is available in Qorx Void. Qorx Void Starter gives new accounts 5,000 included Void/Cloud requests across Windows, macOS, and Linux. Community Edition keeps local source CLI commands unmetered for language, indexing, and evidence work."
    )
}
