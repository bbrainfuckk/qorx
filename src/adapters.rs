...
pub fn science_report() -> ScienceReport {
    ScienceReport {
        ...
        built_in_logic: vec![
            ...
            feature(
                "persistent cosmos store mount",
                "qorx.drive mounts the active Qorx data store as a Windows drive letter and installs a startup remount script; RAM mode requires a real RAM-disk backend",
            ),
            feature(
                "exact replay cache",
                "non-streaming repeat requests can be served locally with x-qorx-cache: hit",
            ),
            feature(
                "provider cached-token parser",
                "gateway parses OpenAI cached_tokens, Anthropic cache_read/cache_creation tokens, and Gemini cachedContentTokenCount",
            ),
            feature(
                "USD savings estimator",
                "stats converts omitted/saved input tokens into estimated USD with QORX_USD_PER_M_INPUT_TOKENS overrides",
            ),
            feature(
                "proxy-first surface",
                "Qorx routes provider traffic through the local proxy and uses session pointers, local qosm state, qshf accounting, cache, and provenance outside model-visible MCP loops. qosm and qshf are runtime labels, not physics claims",
            ),
        ],
