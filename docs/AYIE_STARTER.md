...
Qorx Void Starter is the try-it-for-real path for the paid product.

The idea is simple: people should experience the full Void/Cloud feature set on
Windows, macOS, and Linux before they pay. The starter allowance is 5,000
included Void/Cloud requests. After that, the account asks the user to subscribe
before continuing with Void/Cloud services.
...
These actions count toward the 5,000 included Void/Cloud requests:

- a routed provider call through Qorx Void or Qorx Cloud.
...
Every metered Void/Cloud request should carry an account entitlement and an
...
```text
Qorx Void Starter: 4,812 of 5,000 Void/Cloud requests remaining
```
...
The public repository does not ship the Qorx Void Starter installer binary yet.
...
official Void/Cloud account features, not the public science.

## Metrics

Qorx Void Starter uses local Void runtime metrics plus account-backed service
request counts.
